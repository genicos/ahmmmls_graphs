...
# Loading data
trueanalysis = open("data/recomb_whole_scaling_true_8_2", "r").readlines()
trueanalysis = trueanalysis[1:]
for i in range(len(trueanalysis)):
    trueanalysis[i] = trueanalysis[i].split("\t")

falseanalysis = open("data/recomb_whole_scaling_false_8_2", "r").readlines()
...
# Mapping values
value_to_i = {"0.5":0, "0.8":1, "1.2":2, "2":3}
value = [0.5,0.8,1.2, 2]
...
for line in trueanalysis:
	location_two0[value_to_i[line[1]]].append(float(line[7]) - 0.09*float(line[1]))
	location_two1[value_to_i[line[1]]].append(float(line[9]) - 0.11*float(line[1]))

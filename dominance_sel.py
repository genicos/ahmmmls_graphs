...
fig, axe = plt.subplots(4, 4, figsize=(7,5),constrained_layout=True, sharex=True, sharey='row')


x = [[],[],[],[]]
for i in range(4):
    for k in range(20):
        x[i].append((i+1) - 0.25 + k*0.5/20)
...
for i in range(4):
    for j in range(4):
        ...
        ax.scatter(x[0], dom_sel[i][j][0], s=1, color=color_scheme["blue"])
        ax.scatter(x[0], add_sel[i][j][0], s=1, color=color_scheme["yellow"])
        ax.scatter(x[1], dom_sel[i][j][1], s=1, color=color_scheme["blue"])
        ax.scatter(x[1], add_sel[i][j][1], s=1, color=color_scheme["yellow"])
        ax.scatter(x[2], dom_sel[i][j][2], s=1, color=color_scheme["blue"])
        ax.scatter(x[2], add_sel[i][j][2], s=1, color=color_scheme["yellow"])
        ax.scatter(x[3], dom_sel[i][j][3], s=1, color=color_scheme["blue"])
        ax.scatter(x[3], add_sel[i][j][3], s=1, color=color_scheme["yellow"])

        ax.grid(axis = 'y')
        ax.axhline(y = selections[i], color='black')
...
plt.tight_layout()
plt.subplots_adjust(bottom=0.1, left=0.1, top=0.9, right=0.9, wspace=0.2, hspace=0.2)
